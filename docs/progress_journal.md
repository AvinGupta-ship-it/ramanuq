...
## Day 9 - 2026-06-24
Built the nine-figure suite and the number-injection layer; F3 cleanly shows the [hypothetical] linear-baseline family as most accurate, matching the Day-6 ranking. Verified report_data.json's detection-limit keys reproduce my authored values to rounding. Chose three published [hypothetical material] spectra spanning low-to-high disorder; #1 states height-based analysis so it anchors V5. CX-1 found 0 blockers, 1 deferred nit. Day-9 work commit: 6f05111 

## Day 10 - 2026-06-25
The result I'd defend hardest is the empty ranking — it's counterintuitive but it's the honest output of a floor I set in advance, and it kills the 'you tuned your grid' objection because there's no surviving config to have tuned. V5 at 0.73 proves the pipeline reproduces a real published sp³ within tolerance using the paper's own method; it does not prove the synthetic suite generalizes, which is a separate claim I keep scoped. The hardest scope call was the background-mechanism sentence — I caught myself asserting causation from a confounded factorial and rewrote it as an association, which actually matches what I pre-registered. Writing the disclosure made me realize my ownership lives in the dated decisions and the verification design, not in having typed code. <hash: 2b1b0ba>
...
## 2026-06-28
Finished the v0.2.0 polish: the output-preserving nits (dep pins, F9/F6 data-derived values, dead-code removal, the area/height caveat) were all proven to change zero figure bytes, while the F3/F1(b) label declutter was an intentional presentation change confined to those two figures — the byte-identity split is what keeps the reproducibility claim honest. F10 then visualizes how the minimum detectable change in I_D/I_G falls as ~1/√N_rep when replicate spectra are averaged, with the evidence-based protocol staying below the naive pipeline at every replicate count; critically, each curve's N_rep=1 value re-derives from the same single-spectrum σ and reproduces the frozen per-regime MDCs (0.529/0.271/0.565 vs 0.745/0.763/0.703) rather than copying them, which is the consistency check that the curve isn't fabricated. As with every figure here, this holds only for the configs, spectra, and SNR regimes tested — F10 is a planning aid for sizing replicate counts, not a universal detection-limit guarantee.